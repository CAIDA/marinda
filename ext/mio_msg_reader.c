...
static char message_buf[MIO_MSG_MAX_MESSAGE_SIZE + 1];  /* + NUL-term */
static control_word_t words[MIO_MSG_MAX_WORDS];
static unsigned char decode_buf[MIO_MSG_MAX_RAW_VALUE_SIZE + 1]; /* + NUL*/
...
parse_control_message(const char *message, size_t *length_out)
{
  ...
  for (length = 2; length < MIO_MSG_MAX_WORDS; length++) {
    while (*s && *s == ' ') ++s;
  ...
  /* assert(length >= MIO_MSG_MAX_WORDS); */
...
copy_message(const char *message)
{
  ...
  char *d = message_buf;
  char *dend = &message_buf[MIO_MSG_MAX_MESSAGE_SIZE];
		  ...
		  size_t word_index)
{
  ...
  if (actual_type != expected_type) {
    char name[MIO_MSG_KEYWORD_MAXLEN + 1];
    strncpy(name, words[word_index].cw_name, MIO_MSG_KEYWORD_MAXLEN);
    name[MIO_MSG_KEYWORD_MAXLEN] = '\0';
...
parse_base64_option_value(char *value, size_t word_index)
{
  ...
  if (*s == '\0' || *s == ' ') {
    ...
    if (*s == '\0' || *s == ' ') {
      if (s - (value + 1) > MIO_MSG_MAX_ENCODED_VALUE_SIZE) {
	sprintf(message_buf, "%s/%s option value at pos %d is too long in encoded form; %d bytes of base64 encoding > %d max bytes of encoding",
		...
		MIO_MSG_MAX_ENCODED_VALUE_SIZE);
	return NULL;
      }
      if (*s == ' ') *s++ = '\0';
