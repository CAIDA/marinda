...
static char sprintf_buf[MIO_MSG_MAX_MESSAGE_SIZE + 1];  /* + NUL-term */
...
*/
static char name_buf[MIO_MSG_MAX_MESSAGE_SIZE + 1];  /* + NUL-term */
...
#define APPEND_MSG(s, l)			\
  do { \
    if (l > 0) { \
      if (s + l - message_buf <= MIO_MSG_MAX_MESSAGE_SIZE) { \
        ...
        *s = '\0'; \
		...
		(int)i, MIO_MSG_MAX_MESSAGE_SIZE);			\
	RETURN_ERROR;							\
      } \
    } \
    else { \
			...
			size_t *msg_length_out)
{
  ...
  for (i = 2; i < length; i++) {
	...
	RETURN_ERROR;
      ...
      else if (words[i].cw_len > MIO_MSG_MAX_RAW_VALUE_SIZE) {
	sprintf(message_buf, "%s/%s value is too long in control word at index %d; max length = %d, got %d",
		keyword_type_names[KT_STR], keyword_type_names[KT_BLOB],
		(int)i, MIO_MSG_MAX_RAW_VALUE_SIZE, (int)words[i].cw_len);
	RETURN_ERROR;
      }
